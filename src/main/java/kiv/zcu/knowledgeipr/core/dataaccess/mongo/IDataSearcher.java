...
public interface IDataSearcher {
    ...
    /**
     * Runs an advanced query on the MongoDB database and returns a result set.
     * First a quick query is runAggregation, performing an exact match search, which should be very fast when proper index is used.
     * This query is limited to just a few seconds of execution.
    ...
    /**
     * Runs a simple search on the MongoDB database. The simple search consists of a single query
     ...
     * @param limit - Limit of the returned results
     * @return - Result list of <code>knowledgeipr.DbRecord</code> instances.
     * @throws MongoQueryException            - If there is a problem with a query execution
     * @throws UserQueryException             - If the user query is not in correct format
